...
Функция ПолучитьМеста(ТекстПоиска="") Экспорт
	...
	ТелоЗапроса = СтрШаблон("?apikey=%1&text=%2&lang=az_AZ&print_address=1", Константы.MappableAPIKey.Получить(), ТекстПоиска);
...
Функция ПолучитьКоординатыМеста(Место="") Экспорт
	...
	//@skip-check undefined-variable
	ТелоЗапроса = СтрШаблон("?apikey=%1&geocode=%2&lang=az_AZ", Константы.MappableAPIKey.Получить(), Место);
