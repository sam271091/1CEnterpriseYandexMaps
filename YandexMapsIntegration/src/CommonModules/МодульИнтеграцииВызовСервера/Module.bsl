...
Функция ПолучитьДанныеРасчетаРасстоянияИВремени(НачальнаяТочка, ТочкаНазначения) Экспорт
	...
	//@skip-check undefined-variable
	ТелоЗапроса = СтрШаблон("?origins=%1,%2&destinations=%3,%4&apikey=%5&mode=driving",НачальнаяТочка.Долгота,НачальнаяТочка.Широта,
		ТочкаНазначения.Долгота,ТочкаНазначения.Широта, Константы.MappableAPIKey.Получить());
