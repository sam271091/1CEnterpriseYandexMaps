...
// Получить данные расчета distance matrix.
// 
// Параметры:
//  ИсходнаяТочка Исходная точка
//  ТочкаНазначения Точка назначения
// 
// Возвращаемое значение:
//  Структура - Получить данные расчета distance matrix:
// * Расстояние 
// * Время 
Функция ПолучитьДанныеРасчетаDistanceMatrix(ИсходнаяТочка,ТочкаНазначения) Экспорт
	Результат = МодульИнтеграцииВызовСервера.ПолучитьДанныеРасчетаРасстоянияИВремени(ИсходнаяТочка, ТочкаНазначения);//Структура
