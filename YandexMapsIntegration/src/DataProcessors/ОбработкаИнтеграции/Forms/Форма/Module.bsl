...
&НаСервере
Функция ПолучитьДанныеТочки(Точка)
	...
	|ИЗ
	|	РегистрСведений.КоординатыТранспортныхСредств.СрезПоследних(, ТранспортноеСредство = &Точка) КАК
	|		КоординатыТранспортныхСредствСрезПоследних");
	...
	Возврат ПолучитьКоординатыТочки(Новый Структура("Широта,Долгота",Выборка.Широта,Выборка.Долгота));
	
КонецФункции


&НаСервере
Процедура ПолучитьДанныепоРасстояниюИВремени()
	ДанныеИсходнойТочки = ПолучитьДанныеТочки(ИсходнаяТочка);
	ДанныеТочкиНазначения = ПолучитьДанныеТочки(ТочкаНазначения);
	
	ОбъектОбработки = РеквизитФормыВЗначение("ОбЪект");
	Результат = ОбъектОбработки.ПолучитьДанныеРасчетаDistanceMatrix(ДанныеИсходнойТочки, ДанныеТочкиНазначения);//Структура
	
	Время = Окр(Результат.Время/60,2);
	Расстояние = Окр(Результат.Расстояние/1000,2);
	
КонецПроцедуры

&НаКлиенте
Процедура КомандаПолучитьДанныеМатрицыРасстояний(Команда)
	ПолучитьДанныепоРасстояниюИВремени();
КонецПроцедуры
